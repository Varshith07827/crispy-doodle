...
class FakeController:
    ...
    def set_chat_memory_mongo(self, uri, database=""):
        ...
        if connected or not uri:
            self.mongo_problem = ""   # a good connect clears the last failure
        return self.chat_memory_backend_name, migrated

    def chat_memory_last_error(self):
        return self.mongo_problem
...
# --- AI command mode ("!winspark …") ------------------------------------------

def _select_command_mode(panel, word="winspark"):
    _select_openai(panel)
    panel._ai_mode.setCurrentIndex(panel._ai_mode.findData("command"))
    panel._ai_command.setText(word)


def test_command_word_field_shows_only_in_command_mode(whatsapp):
    _select_openai(whatsapp)

    whatsapp._ai_mode.setCurrentIndex(whatsapp._ai_mode.findData("reply"))
    assert whatsapp._ai_command_row.isHidden() is True
    whatsapp._ai_mode.setCurrentIndex(whatsapp._ai_mode.findData("command"))
    assert whatsapp._ai_command_row.isHidden() is False
    whatsapp._ai_mode.setCurrentIndex(whatsapp._ai_mode.findData("generate"))
    assert whatsapp._ai_command_row.isHidden() is True


def test_starting_command_mode_saves_the_command_word(whatsapp, controller):
    whatsapp._chat_name.setText("Family")
    _select_command_mode(whatsapp)
    whatsapp.toggle_automation()

    assert controller.last_start_kwargs["reply_source"] == "openai"
    assert controller.last_start_kwargs["ai_mode"] == "command"
    assert controller.last_start_kwargs["trigger_text"] == "winspark"


def test_command_word_typed_with_the_prefix_is_cleaned(whatsapp, controller):
    """"!winspark" in the box would otherwise be stored as the name, making the
    matcher hunt for "!!winspark"."""
    whatsapp._chat_name.setText("Family")
    _select_command_mode(whatsapp, word="!winspark")
    whatsapp.toggle_automation()

    assert controller.last_start_kwargs["trigger_text"] == "winspark"


def test_plain_reply_mode_stores_no_command_word(whatsapp, controller):
    """A leftover word in the box must not turn plain AI reply into command mode."""
    whatsapp._chat_name.setText("Family")
    _select_command_mode(whatsapp)
    whatsapp._ai_mode.setCurrentIndex(whatsapp._ai_mode.findData("reply"))
    whatsapp.toggle_automation()

    assert controller.last_start_kwargs["ai_mode"] == "reply"
    assert controller.last_start_kwargs["trigger_text"] == ""


def test_command_mode_refuses_to_start_without_a_word(whatsapp, controller):
    """Starting with an empty word would build a binding that can never fire."""
    whatsapp._chat_name.setText("Family")
    _select_command_mode(whatsapp, word="   ")
    before = len(controller.started)
    whatsapp.toggle_automation()

    assert len(controller.started) == before      # nothing started
    assert whatsapp._chat_check.state == "bad"
    assert "!winspark" in whatsapp._chat_check.message


def test_saved_command_automation_is_described_in_plain_english():
    from winspark.ui.panels import _describe_binding_method

    binding = WhatsAppFetchBindingEntity(
        group_name="Papa", reply_source="openai", ai_mode="command", trigger_text="winspark")
    assert _describe_binding_method(binding) == 'AI answers "!winspark"'


# --- the Settings panel (app-wide AI service) ---------------------------------

def _settings_panel(controller):
    """A Settings panel whose MongoDB connect runs inline — production does it
    on a worker thread (an Atlas connect can take seconds)."""
...
def test_settings_mongo_failure_shows_the_actual_reason(qapp, controller):
    """An Atlas IP-allowlist or bad-password failure must say so — "couldn't
    reach MongoDB" gives the user nothing to act on."""
    controller.mongo_reachable = False
    controller.mongo_problem = ("Couldn't reach the Atlas cluster. Check that this machine's IP "
                                "is on the cluster's Network Access allowlist.")
    panel = _settings_panel(controller)
    panel._mongo_uri.setText("mongodb+srv://u:p@cluster.mongodb.net/")
    panel.save_chat_memory()

    assert panel._mongo_check.state == "bad"
    assert "Network Access allowlist" in panel._mongo_check.message


def test_settings_mongo_connect_names_the_database_in_use(qapp, controller):
    """The database can come from the URI rather than the field, so the
    confirmation states which one actually got used."""
    controller.mongo_reachable = True
    controller.mongo_database = "Winsparkpro"
    panel = _settings_panel(controller)
    panel._mongo_uri.setText("mongodb://localhost:27017/Winsparkpro")
    panel.save_chat_memory()

    assert panel._mongo_check.state == "ok"
    assert "database: Winsparkpro" in panel._mongo_check.message


def test_settings_panel_shows_startup_mongo_failure_on_open(qapp, controller):
    """A URI saved but unreachable at startup shouldn't look like the user
    chose local storage — the panel says what went wrong on sight."""
    controller.mongo_uri = "mongodb+srv://u:p@cluster.mongodb.net/"
    controller.mongo_problem = "MongoDB rejected the username and password in the connection string."
    panel = _settings_panel(controller)
