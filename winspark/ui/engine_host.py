...
class EngineHost:
    def __init__(self, connection_factory: ConnectionFactory) -> None:
        ...
        self._chat_memory, self._chat_memory_problem = try_build_chat_memory_store(
    ...
    def chat_memory_last_error(self) -> str:
        """Why MongoDB was skipped, in plain English — empty when it connected
        (or when no URI is configured, which isn't an error). Lets the Settings
        panel say what to fix instead of a bare "couldn't connect"."""
        return self._chat_memory_problem

    def chat_memory_database(self) -> str:
    ...
    def set_chat_memory_mongo(self, uri: str, database: str = "") -> tuple[str, int]:
        ...
        new_store, self._chat_memory_problem = try_build_chat_memory_store(
            uri, self._repository, database=database)
        migrated = 0
