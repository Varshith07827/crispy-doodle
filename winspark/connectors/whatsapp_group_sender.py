...
class WhatsAppGroupSender:
    ...
    async def _send_to_group_locked(self, group_name: str, message_text: str) -> WhatsAppGroupSendResult:
        ...
        if not typed:
            # Clear up after a PARTIAL type too. Returning straight out used to
            # leave whatever landed sitting in the box: the user found a long
            # half-written answer parked in the chat, and the next attempt
            # appended to it. Only the send-loop's exit path cleaned up.
        ...
        for attempt in range(3):
            ...
            if attempt < 2:
                ...
                # multi-line answer, so every attempt retyped the whole thing
                # (21s for a long one) instead of just pressing Send again.
...
def _read_clipboard_text() -> Optional[str]:
    """The clipboard's text, or None if it holds something else / can't be read."""
    for _ in range(5):
        try:
...
def _paste_text(compose, text: str) -> bool:
    """Put `text` into the focused compose box via the clipboard.

    Pasting sidesteps three problems that per-character typing has with a long
    message: the 30ms-per-character cost, the chance of a dropped keystroke
    growing with length, and newlines — which are typed as a Unicode U+000A
    keystroke that WhatsApp's compose box does not reliably turn into a line
    break. A paste inserts the text verbatim, newlines and emoji included.

    The user's own clipboard is put back afterwards. Non-text clipboard
    contents (an image, say) can't be preserved this way and are lost — the
    trade for being able to send long messages at all.

    Returns False if anything goes wrong, so the caller can fall back to
    typing; this never raises."""
    try:
        ...
        logger.warning("Pasting into the compose box failed — falling back to typing", exc_info=True)
        return False
...
def _set_compose_text_sync(window_handle: int, text: str) -> bool:
    ...
    try:
        # Already right (a retry after a send that didn't take)? Leave it alone —
        # re-typing risks losing it or doubling it up.
