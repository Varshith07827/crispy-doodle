...
**Call it by name, like an in-chat assistant.** Set an AI automation to *"Only when
someone types !name"* and winSpark stays silent until it's addressed — then anyone
in the chat writes `!winspark what's the weather` and it answers that question, with
the chat's memory behind it. Everything else said in the chat is ignored, so it never
joins in uninvited. The name is matched exactly (`!winsparkling` and
`docs/winspark/readme.md` don't call it), and each call is answered once.
...
**Chat memory storage.** What winSpark remembers per chat lives in a local SQLite
file by default. Paste a MongoDB connection string to keep it in your own database
too — either a **local Community Server** (`mongodb://localhost:27017`) or a **cloud
Atlas cluster** (`mongodb+srv://…mongodb.net`). Both stores are written on every
message and reconciled on connect, so they never diverge; MongoDB keeps the *full*
history while the local copy stays bounded. If MongoDB is unreachable the app falls
back to local storage and tells you why.
